...
export const AddAssessor = () => {
  ...
  const handleSubmit = (e) => {
    ...
    apiFormData.append('role', 'ASSESSOR');
    ...
    registerTutor(apiFormData)
      .then((res) => {
        if (res.success) {
          setMsg("Assessor created successfully!");
          e?.target?.reset();
        } else {
          setMsg(res?.errMsg || res?.message || "Error creating assessor");
        }
      })
      .catch((err) => {
        setMsg("Error creating assessor. Please try again.");
        console.error(err);
      })
      .finally(() => {
        setLoading(false);
    ...
    <div className="mt-8 bg-[#0d1e3a] min-h-screen">
      <form onSubmit={handleSubmit} className="w-8/12 mx-auto">
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? "Creating..." : "Create Assessor"}
        </button>
