...
export const getEnrolledBootcamps = ({ page = 1, limit = 4, track = "all", archived = false } = {}) => {
  const token = localStorage.getItem("TOKEN");
  const headers = token ? { "auth-token": token } : {};
  const params = { page, limit, track, archived: archived ? "true" : "false" };
 ...
 */
export async function getEnrolledBootcampsWithPolling({ page = 1, limit = 12, track = "all", archived = false } = {}, opts = {}) {
  const { onPoll } = opts;
  const token = localStorage.getItem("TOKEN");
  const authHeaders = token ? { "auth-token": token } : {};
  const body = { page, limit, track, archived: archived ? "true" : "false" };
  try {
    ...
    if (!startRes.data?.success || !startRes.data?.jobId) {
      return getEnrolledBootcamps({ page, limit, track, archived });
    }
